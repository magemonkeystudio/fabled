<script lang="ts">
  ...
  const dragover = (e: DragEvent) => {
    if (
      ...
      !(e.dataTransfer.types.length > 0 && e.dataTransfer?.types.includes('Files'))
    )
      return;
...
<div id="floating-buttons">
  ...
  {#if $socketConnected}
    ...
    <div
      ...
      onkeypress={(e) => {
        if (e.key === 'Enter') exportAllToServer();
      }}
    >
...
{#if $saveError}
  <div class="save-error" transition:fly={{ y: -20 }}>
    <strong>Failed to save {$saveError.name}.</strong>
    <div>{$saveError.message}</div>
    <div>Export before refreshing or closing if you need to keep working with this data.</div>
    <div
...
{#if ModalService.activeModal}
  <ModalService.activeModal
    data={$modalData as FabledComponent}
    onclose={closeModal}
    onsave={save}
  />
{/if}
