...
public class ProAttribute implements IconHolder {
    ...
    private String filter(PlayerData data, String text) {
        return text
                ...
                .replace("{max}", String.valueOf(this.getMax())) // iomatix: the limit, to verify: is it conflicting with skills' {max} ?
    ...
    private void loadGroup(DataSection data, ComponentType type) {
        ...
        for (String key : data.keys()) {
            ...
            SkillAPI.getAttributeManager().addByComponent(lower, this);
        }
    }
    ...
    private void loadStatModifier(DataSection data, String key) {
        if (data.has(key)) {
            ...
            SkillAPI.getAttributeManager().addByStat(key, this);
        }
    }
}