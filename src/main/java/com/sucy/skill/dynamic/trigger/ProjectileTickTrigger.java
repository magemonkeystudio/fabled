...
public class ProjectileTickTrigger implements Trigger<ProjectileTickEvent> {
    ...
    public boolean shouldTrigger(ProjectileTickEvent event, int level, Settings settings) {
        ...
        boolean isCorrectProjectile = projectiles.isEmpty() || projectiles.contains("Any") || projectiles.stream().anyMatch(projectile->event.getProjectile().getType().name().equalsIgnoreCase(projectile));
        if (!isCorrectProjectile) return false;
