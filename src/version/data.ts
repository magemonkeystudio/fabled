...
export const versionData: Writable<VersionData> = writable(VERSIONS[<Versions>parseInt(versionKeys[0])]);

export const serverOptions: string[] = [];
versionKeys.forEach((v: string) => serverOptions.push('1.' + v));
export const version: Writable<Versions> = localStore<Versions>('server-version', <Versions>parseInt(versionKeys[0]));
version.subscribe((ver: Versions) => {
	if (!(ver in VERSIONS)) {
		ver = <Versions>parseInt(versionKeys[0]);
		version.set(ver);
		return;
	}
