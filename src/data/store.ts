...
export const active: Writable<FabledClass | FabledSkill | FabledAttribute | undefined> =
  writable(undefined);
...
export const dragging: Writable<FabledClass | FabledSkill | FabledAttribute | FabledFolder> =
  writable();
...
export const localSyncList: Writable<
  Map<FileSystemFileHandle, FabledSkill | FabledClass | FabledAttribute>
> = writable(new Map());
...
const addSyncLocal = async (data: FabledSkill | FabledClass | FabledAttribute) => {
  ...
  const file = await window.showOpenFilePicker({
    types: [
      { description: 'Choose a yaml file to sync', accept: { 'text/yaml': ['.yml', '.yaml'] } }
    ],
    multiple: false,
...
const findFileHandle = (data: FabledSkill | FabledClass | FabledAttribute) => {
  return get(localSyncList)
    .entries()
    .find(([_, value]) => value === data)?.[0];
};

export const saveError: Writable<{ name: string; message: string } | undefined> = writable();
