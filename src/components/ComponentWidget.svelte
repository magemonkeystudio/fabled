<script lang="ts">
	...
	let { skill, component }: Props = $props();
	...
	const addSkill = (e: {
		detail: { comp: FabledComponent; relativeTo: FabledComponent; above: FabledComponent };
	}) => {
...
<div class="wrapper">
	<div
		...
		ondragstart={stopPropagation(() => draggingComponent.set(component))}
		ondragend={() => draggingComponent.set(undefined)}
		ondrop={stopPropagation(drop)}
		onclick={stopPropagation(openCompModal)}
		onkeypress={(e) => {
			if (e.key === 'Enter') {
				e.stopPropagation();
				openCompModal();
	...
	>
		...
		<div
			...
			tabindex="0"
			role="button"
			onclick={stopPropagation(() => (collapsed = !collapsed))}
			onkeypress={(e) => {
				if (e.key === 'Enter') {
					e.stopPropagation();
					collapsed = !collapsed;
				...
				{#if childrenList.length === 0}
					{#if component.isParent && (over || overChildren)}
						<div
							...
							ondragover={stopPropagation(preventDefault(() => {}))}
							ondragleave={leave}
