<script lang="ts">
	...
	const addComponent = (comp: { new: () => { defaultOpen: () => FabledComponent } }) => {
...
<Modal width="70%" open={true} on:close={closeModal}>
		...
		{#if conditionsShown}
			<div class="component-section" transition:squash={{ duration: 200 }}>
				{#each $filteredConditions as condition}
					...
					>
						{#if condition.component.new().isDeprecated}<s>{condition.name}</s
