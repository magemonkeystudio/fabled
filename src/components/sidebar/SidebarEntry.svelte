<script lang="ts">
  ...
  let { delay = 0, direction = 'left', data = undefined, children, onclick }: Props = $props();
  ...
  {#if data}
    <div class="buttons">
      {#if data instanceof FabledSkill}
        <a href="{base}/skill/{data.name}/edit" class="edit" title="Edit Skill">
      ...
      {#if !browser || 'showOpenFilePicker' in window}
        {#key sync}
          <div
            onclick={(e) => toggleSyncLocal(data).then(() => (sync = isSyncLocal(data)))}
            onkeypress={(event) => {
              if (event?.key === 'Enter')
                toggleSyncLocal(data).then(() => (sync = isSyncLocal(data)));
            }}
            class:activeSync={sync}
            tabindex="0"
            role="button"
      ...
      <div
        onclick={(e) => saveData(data, e)}
        onkeypress={(event) => {
          if (event?.key === 'Enter') saveData(data, event);
        }}
        tabindex="0"
        role="button"
        class="download"
      ...
      <div
        onclick={(e) => cloneData(data, e)}
        onkeypress={(event) => {
          if (event?.key === 'Enter') cloneData(data, event);
        }}
        tabindex="0"
        role="button"
        class="clone"
  ...
  <Modal>
    ...
    <div class="modal-buttons">
      <div
        ...
        onkeypress={(event) => {
          if (event?.key === 'Enter') deleting = false;
        }}
        role="button"
        tabindex="0"
      >
      ...
      <div
        ...
        onclick={() => deleteProData(data)}
        onkeypress={(event) => {
          if (event?.key === 'Enter') deleteProData(data);
        }}
        role="button"
        tabindex="0"
      >
...
<style>
  ...
  :global(.folder-content .sidebar-entry) {
