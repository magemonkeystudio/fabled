...
function Class(name) {
    ...
    this.data = [
        ...
        new ListValue('Permission', 'needs-permission', ['True',
                                                         'False'], 'False').setTooltip('Whether or not the class requires a permission to be professed as. The permission would be "skillapi.class.{className}"'),
        new ByteListValue('Exp Sources', 'exp-source', ['Mob',
                                                        'Block Break',
                                                        'Block Place',
                                                        'Craft',
                                                        'Command',
                                                        ...
                                                        'Exp Bottle',
                                                        'Smelt',
                                                        'Quest'], 273).setTooltip('The experience sources the class goes up from. Most of these only work if "use-exp-orbs" is enabled in the config.yml.'),
        ...
        new ListValue('Skill Tree', 'tree', ['Basic Horizontal',
                                             'Basic Vertical',
                                             'Level Horizontal',
                                             'Level Vertical',
                                             'Flood',
...
Class.prototype.updateAttribs = function (i) {
    ...
    for (j = 0; j < ATTRIBS.length; j++) {
        let attrib = ATTRIBS[j].toLowerCase();
        let format = attrib.charAt(0).toUpperCase() + attrib.substr(1);
...
Class.prototype.createFormHTML = function () {
    ...
    for (let i = 0; i < this.data.length; i++) {
        ...
        if (this.data[i].name == 'Mana') {
            let dragInstructions       = document.createElement('label');
    ...
    let save = document.createElement('h5');
    ...
    let target       = document.getElementById('classForm');
    target.innerHTML = '';
